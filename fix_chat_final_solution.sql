-- FINAL CHAT FIX - Complete elimination of circular references
-- This approach uses the simplest possible policies to avoid any recursion
...
-- Step 4: Create ULTRA SIMPLE policies with NO circular references
-- These policies only check the user's own data, nothing else

-- Chat Group Members - Only user's own memberships
CREATE POLICY "user_memberships_only" ON chat_group_members
    FOR ALL TO authenticated
    USING (user_id = auth.uid())
    WITH CHECK (user_id = auth.uid());

-- Chat Groups - Only groups created by user
CREATE POLICY "user_groups_only" ON chat_groups
    FOR ALL TO authenticated
    USING (creator_id = auth.uid())
    WITH CHECK (creator_id = auth.uid());

-- Chat Messages - Only messages sent by user
CREATE POLICY "user_messages_only" ON chat_messages
    FOR ALL TO authenticated
    USING (sender_id = auth.uid())
    WITH CHECK (sender_id = auth.uid());

-- Chat Invitations - Only invitations involving user
CREATE POLICY "user_invitations_only" ON chat_group_invitations
    FOR ALL TO authenticated
    USING (invitee_id = auth.uid() OR inviter_id = auth.uid())
    WITH CHECK (inviter_id = auth.uid());

-- Step 5: Create SEPARATE policies for group access (no circular references)
-- These policies use direct table queries without joins to avoid recursion

-- Allow users to see groups they're members of (direct query)
CREATE POLICY "member_group_access" ON chat_groups
...
-- Allow users to see messages in groups they're members of (direct query)
CREATE POLICY "member_message_access" ON chat_messages
    FOR SELECT TO authenticated
    USING (
        group_id IN (
            SELECT group_id 
...
-- Allow users to see invitations for groups they're members of (direct query)
CREATE POLICY "member_invitation_access" ON chat_group_invitations
    FOR SELECT TO authenticated
    USING (
        group_id IN (
            SELECT group_id 
...
-- Step 6: Fix the create_or_get_direct_chat function with proper permissions
