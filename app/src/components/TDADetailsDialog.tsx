...
import { Download, X, Calendar, Clock, User, TrendingUp, Image as ImageIcon, Bell, Brain } from 'lucide-react';
...
export default function TDADetailsDialog({ isOpen, onClose, analysisId }: TDADetailsDialogProps) {
  ...
  useEffect(() => {
    if (isOpen && analysisId && analysisId.trim() !== '') {
      fetchAnalysisData();
    }
  }, [isOpen, analysisId]);
  ...
  const getHardcodedQuestions = (timeframe: string) => {
    if (timeframe === 'DAILY') {
      ...
      return [
        ...
        { id: 'min15_macd_lines_waterline', question_text: 'MACD Lines: Waterline' },
        { id: 'min15_macd_lines_position', question_text: 'MACD Lines: Position' },
        { id: 'min15_macd_lines_movement', question_text: 'MACD Lines: Movement' },
        { id: 'min15_macd_lines_sentiment', question_text: 'MACD Lines: Sentiment' },
        { id: 'min15_macd_lines_notes', question_text: 'Notes' },
        { id: 'min15_macd_histogram_waterline', question_text: 'MACD Histogram: Position' },
        { id: 'min15_macd_histogram_movement', question_text: 'MACD Histogram: Movement' },
        { id: 'min15_macd_histogram_sentiment', question_text: 'MACD Histogram: Sentiment' },
        { id: 'min15_macd_histogram_notes', question_text: 'Notes' },
        { id: 'min15_rsi_condition', question_text: 'RSI: Condition' },
        { id: 'min15_rsi_direction', question_text: 'RSI: Direction' },
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto bg-white/80 backdrop-blur-sm border-0 shadow-2xl">
        <DialogHeader className="bg-gradient-to-r from-orange-50 to-amber-50 p-6 rounded-t-lg border-b border-orange-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-slate-600 mt-1">
                  {data.analysis?.currency_pair} • {formatDateTime(data.analysis?.analysis_date, data.analysis?.completed_at)}
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <Button
                onClick={async () => {
                  try {
                    const response = await fetch('/api/tda/fix-timeframes', {
                      method: 'POST',
                    ...
                    if (response.ok) {
                      ...
                      toast({
                        title: "Success",
                        description: `Fixed ${result.timeframes_created} timeframes`,
                      });
                      // Refresh the data
                      fetchAnalysisData();
                    } else {
                      throw new Error('Failed to fix timeframes');
                    }
                  } catch (error) {
                    toast({
                      title: "Error",
                      description: "Failed to fix timeframes",
                      variant: "destructive"
                    });
                  }
                }}
            ...
            <CardContent className="space-y-4">
              {analyzedTimeframes.length === 0 ? (
                ...
                analyzedTimeframes.map((timeframe: string) => {
                  ...
                  const hardcodedQuestions = getHardcodedQuestions(timeframe);
                  ...
                  return (
                    <div key={timeframe} className="bg-white/70 rounded-lg p-4 border border-green-200">
                      ...
                      <div className="space-y-3">
                        {hardcodedQuestions.map((question: any) => {
