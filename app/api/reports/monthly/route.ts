...
export async function POST() {
  try {
    ...
    const { data: users, error: usersError } = await supabase
      .from('user_settings')
      .select('user_id, monthly_reports')
      .eq('monthly_reports', true);

    if (usersError) {
      console.error('Error fetching users with monthly reports enabled:', usersError);
    ...
    if (!users || users.length === 0) {
      return NextResponse.json({ 
        message: "No users have monthly reports enabled" 
      });
    }
    ...
    const { startDate, endDate } = getDateRangeForReport('monthly');
    const periodLabel = getReportPeriodLabel('monthly');
    ...
    for (const user of users) {
      try {
        ...
        // Generate filename
        const fileName = getReportFileName('monthly', userProfile.username || user.user_id);

        // Prepare email content
        const emailSubject = `Your Monthly Trade Report - ${periodLabel}`;
        const emailHtml = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <div style="background-color: #f5f5f5; padding: 20px; text-align: center;">
              <h1 style="color: #333; margin: 0;">Trader's Journal</h1>
              <p style="color: #666; margin: 5px 0 0 0;">Monthly Trade Report</p>
            </div>
            <div style="padding: 20px; border: 1px solid #ddd; border-top: none;">
              <h2 style="color: #333; margin-top: 0;">Your Monthly Trade Report</h2>
              <p style="color: #333; font-size: 16px;">
                Here's your monthly trade report for <strong>${periodLabel}</strong>.
              </p>
              
              ${monthlyTrades && monthlyTrades.length > 0 ? `
                <div style="background-color: #f9f9f9; padding: 15px; border-radius: 5px; margin: 20px 0;">
                  ...
                  <p style="margin: 5px 0;"><strong>Total Trades:</strong> ${monthlyTrades.length}</p>
                  <p style="margin: 5px 0;"><strong>Report Type:</strong> Monthly Trade Report</p>
                </div>
              ` : `
                <div style="background-color: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0; border: 1px solid #ffeaa7;">
                  <h3 style="margin: 0 0 10px 0; color: #856404;">No Trades This Month</h3>
                  <p style="margin: 5px 0; color: #856404;">
                    You didn't record any trades during ${periodLabel}. 
                    This is completely normal - every trader has quiet months.
                  </p>
                  <p style="margin: 10px 0 0 0; color: #856404; font-size: 14px;">
                    When you start recording trades again, you'll receive detailed monthly reports with Excel attachments.
                  </p>
                </div>
              `}
              ...
              <p style="color: #666; font-size: 14px;">
                You're receiving this email because you have monthly trade reports enabled.
                <br>
        ...
        console.error(`Error processing monthly report for user ${user.user_id}:`, error);
        errorCount++;
    ...
    console.error('Error in monthly reports endpoint:', error);
    return NextResponse.json({ 
